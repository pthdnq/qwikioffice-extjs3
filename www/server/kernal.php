...
class kernal {
   ...
   private function print_module_locale($module_id){
      ...
      // get the members locale
      $member_locale = $this->get_member_locale($member_id);
      if(!$member_locale){
         return false;
      }

      // get the module locale file
      $this->load('module');
      ...
      // is the locale file a regular file?
      $document_root = $this->get_document_root();
      $module_dir = $document_root.$this->get_module_dir();

      $file = $module_dir.$module->locale->directory.$language.$module->locale->extension;

      // put the file contents (JSON) into a string
      $string = file_get_contents($file);
      if($string){
         print $module->locale->class.' = '.$string.';';
      }
