	...
	constructor : function(config){
		...
		Ext.applyIf(config, {
			...
			, items: [
				...
				, new QoDesk.QoPreferences.Shortcuts.Tree({
	    ...
	    function onSave(){
	    	...
	    	this.ownerModule.save({
	    		...
	    		, callbackScope: this
	    		, ids: Ext.encode(this.ownerModule.app.launchers.shortcut)
	    	});
	    }
	}
});

QoDesk.QoPreferences.Shortcuts.Tree = Ext.extend(Ext.tree.TreePanel, {
	constructor : function(config){
		// constructor pre-processing
		config = config || {};
		...
		var ids = this.ownerModule.app.launchers.shortcut;
		...
		QoDesk.QoPreferences.Shortcuts.Tree.superclass.constructor.apply(this, [config]);
		// constructor post-processing
		...
		function onCheckChange(node, checked){
			if(node.leaf && node.id){
	    		if(checked){
					this.ownerModule.app.desktop.addShortcut(node.id, true);
	    		}else{
					this.ownerModule.app.desktop.removeShortcut(node.id, true);
	    		}
	    	}
