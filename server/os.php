...
class os extends kernal {
   ...
   public function print_theme_css(){
      ...
      // get the theme
      $preference = $this->get_member_preference($member_id, $group_id);
      if(isset($preference->themeId)){
         $theme_id = $preference->themeId;
      }
      ...
      $file = $this->theme->get_file($theme_id);
      ...
      if(!is_file($document_root.$theme_dir.$file)){
         // log error
         $this->load('log');
		   $this->log->error('Line number 337 of os.php, Missing theme file ('.$theme_dir.$file.')');
   ...
   public function login($user, $pass, $group_id = ''){
      ...
      $passResult = $this->db->conn->query($passSql);
      if($passResult){
         $hashedPass = $passResult->fetchColumn();
         $pass = $this->security->dataHash($pass, $hashedPass);
      }

      // do we have a successful login?
      $member_id = $this->member->get_id($user, $pass);
      if(!$member_id){
